...
class VersionManager:
    ...
    def _update_app_version(self, version: str, info: Dict):
        ...
        content = re.sub(
            r'__release_date__ = ["\'][^"\']+["\']',
            f'__release_date__ = "{info["release_date"]}"',
            content
        )
    ...
    def _update_readme_version(self, version: str):
        ...
        content = re.sub(
            r'# Second Brain v[\d.]+',
            f'# Second Brain v{version}',
            content
        )
        
        # Update architecture overview
        content = re.sub(
            r'Second Brain v[\d.]+ represents',
            f'Second Brain v{version} represents',
            content
        )
...
"""
    ...
    def _update_project_status(self, version: str, info: Dict):
        ...
        content = re.sub(
            r'\*\*Version\*\*: [^\n]+',
            f'**Version**: {version} ({info["status"].title()})',
            content
        )
        
        content = re.sub(
            r'\*\*Last Updated\*\*: [^\n]+',
            f'**Last Updated**: {info["release_date"]}',
            content
        )
...
"""
    ...
    def generate_git_commands(self, version: str, info: Dict):
        ...
        if info["git_workflow"] == "testing->main->release":
            ...
            if info.get("pr_testing", False):
                ...
                print("gh pr create --title \"Release v{version}\" --body \"See RELEASE_NOTES_v{version}.md for details\" --base main --head testing")
                print()
            ...
            print("gh pr create --title \"Release v{version}\" --body \"See RELEASE_NOTES_v{version}.md for details\" --base main --head develop")
        ...
        print(f"gh release create v{version} --title \"Second Brain v{version}\" --notes-file docs/releases/RELEASE_NOTES_v{version}.md")
    ...
    def run_test_suite(self, test_type: str = "all") -> bool:
        ...
        try:
            import subprocess
            result = subprocess.run(test_commands[test_type], capture_output=True, text=True, encoding='utf-8', errors='ignore')
