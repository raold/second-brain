...
def check_system_resources():
    ...
    if memory.available < 2 * 1024 * 1024 * 1024:  # 2GB
        print(f"⚠️  Warning: Low available memory ({memory.available / 1024 / 1024 / 1024:.1f}GB). Load testing may fail.")
        return False

    # Check CPU
    cpu_count = psutil.cpu_count()
    if cpu_count < 2:
        print(f"⚠️  Warning: Low CPU count ({cpu_count}). Load testing may be limited.")

    # Check disk space
    disk = psutil.disk_usage('.')
    if disk.free < 1024 * 1024 * 1024:  # 1GB
        print(f"⚠️  Warning: Low disk space ({disk.free / 1024 / 1024 / 1024:.1f}GB free).")
