...
from app.batch_classification_engine import BatchClassificationEngine, ClassificationConfig, ClassificationMethod
from app.bulk_memory_manager import BulkMemoryManager, ExportFormat, ImportFormat
...
from app.memory_migration_tools import MigrationConfig, MigrationManager
...
async def demo_bulk_import_export():
    ...
    import_result = await bulk_manager.import_memories(
        data=sample_memories, format_type=ImportFormat.JSON, options={"check_duplicates": True}
    )
...
"""
    ...
    csv_import_result = await bulk_manager.import_memories(
        data=csv_data, format_type=ImportFormat.CSV, options={"has_header": True, "delimiter": ","}
    )
    ...
    for format_type, format_name in export_formats:
        try:
            ...
            export_result = await bulk_manager.export_memories(
                filter_criteria={"memory_type": "semantic"}, format_type=format_type, options={}
            )
            export_time = time.time() - start_time

            print(
                f"   ✅ {format_name}: {export_result.total_exported} memories, "
...
async def demo_memory_migration():
    ...
    migration_id = migration_manager.create_memory_type_migration(
        from_type="semantic", to_type="knowledge", filter_criteria={"content_contains": "programming"}
    )
    ...
    config = MigrationConfig(
        batch_size=10, backup_before_migration=True, validate_before_execution=True, rollback_on_failure=True
    )
...
async def demo_batch_classification():
    ...
    sample_memories = [
        {
            "id": "1",
            "content": "The capital of France is Paris. It's located in the northern part of the country.",
            "metadata": {},
        },
        {
            "id": "2",
            "content": "Yesterday I went to the grocery store and bought apples, bread, and milk.",
            "metadata": {},
        },
        {
            "id": "3",
            "content": "To bake a cake: 1) Preheat oven to 350°F, 2) Mix dry ingredients, 3) Add wet ingredients, 4) Bake for 30 minutes.",
            "metadata": {},
        },
        {
            "id": "4",
        ...
        {
            "id": "5",
            "content": "I remember the first time I rode a bicycle. I was seven years old and my father taught me in the park.",
            "metadata": {},
        },
        {
            "id": "6",
            "content": "How to install Python: 1) Download from python.org, 2) Run the installer, 3) Add to PATH, 4) Verify installation.",
            "metadata": {},
        },
    ]
    ...
    for method, method_name in methods:
        ...
        config = ClassificationConfig(
            method=method, batch_size=10, confidence_threshold=0.5, parallel_workers=2, enable_caching=True
        )
        ...
        for classification in result.classification_results[:3]:  # Show first 3
            print(
                f"      Memory {classification.memory_id}: {classification.predicted_type} "
...
async def demo_memory_deduplication():
    ...
    sample_memories = [
        ...
        {
            "id": "7",
            "content": "How to install Python packages with pip",
        ...
        if result.duplicate_groups:
            ...
            for group in result.duplicate_groups[:2]:  # Show first 2 groups
                ...
                for score in group.similarity_scores[:1]:  # Show first similarity
                    print(
                        f"        {score.memory_id_1} ↔ {score.memory_id_2}: "
...
async def demo_integrated_workflow():
    ...
    import_result = await bulk_manager.import_memories(data=diverse_memories, format_type=ImportFormat.JSON)
    ...
    config = ClassificationConfig(
        method=ClassificationMethod.HYBRID, confidence_threshold=0.6, auto_apply_results=False
    )
    ...
    export_result = await bulk_manager.export_memories(
        format_type=ExportFormat.JSON, options={"include_metadata": True}
    )
