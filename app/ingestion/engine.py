...
class TextFileParser(FileParser):
    ...
    SUPPORTED_TYPES = {"text/plain", "text/markdown", "text/x-markdown", "application/x-markdown"}
    ...
    async def parse(self, file_path: Path) -> dict[str, Any]:
        ...
        return {"content": content, "metadata": {"format": "text", "encoding": "utf-8"}}
...
class IngestionEngine:
        ...
        try:
            ...
            return IngestionResult(
                ...
                file_metadata=(
                    ...
                    else FileMetadata(filename=filename, file_type="unknown", size=0, hash="")
                ),
                errors=[str(e)],
                processing_time=asyncio.get_event_loop().time() - start_time,
            )
    ...
    async def _save_temp_file(self, file: Union[BinaryIO, Path, str], filename: str) -> Path:
    ...
    def _validate_file(self, file_metadata: FileMetadata):
        ...
        if file_metadata.size > self.max_file_size:
            raise ValueError(
                f"File too large: {file_metadata.size} bytes " f"(max: {self.max_file_size} bytes)"
            )
    ...
    async def ingest_google_drive_file(
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                suffix=Path(file_info.name).suffix, delete=False
            ) as tmp_file:
    ...
    async def batch_ingest(
        self, files: list[dict[str, Any]], user_id: str, tags: list[str] | None = None
