...
class StructuredDataExtractor:
    ...
    def _extract_structured_tables(self, text: str) -> list[dict[str, Any]]:
        ...
        if potential_rows:
            ...
            for keys, rows in key_groups.items():
                if len(rows) >= 2:
                    tables.append({"headers": list(keys), "rows": rows, "type": "structured"})
    ...
    def _detect_language(self, code: str) -> str:
        ...
        # SQL
        if any(
            keyword in code.upper() for keyword in ["SELECT", "INSERT", "UPDATE", "DELETE", "FROM"]
        ):
            return "sql"
    ...
    def get_extraction_statistics(self, structured_data: StructuredData) -> dict[str, Any]:
        ...
        stats = {
            ...
            "total_code_lines": sum(
                snippet.get("lines", 0) for snippet in structured_data.code_snippets
            ),
            "code_languages": list(
                set(snippet.get("language", "unknown") for snippet in structured_data.code_snippets)
            ),
