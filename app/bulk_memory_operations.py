...
class BulkMemoryEngine:
    ...
    async def bulk_insert_memories(
        ...
        try:
            ...
            if hasattr(self.database, 'pool') and self.database.pool:
                ...
                memory_ids = await self._process_insert_batches_mock(
                    valid_items, progress, config, error_summary
                )
        ...
        if hasattr(self.database, 'pool') and self.database.pool:
            try:
                async with self.database.pool.acquire() as conn:
                    ...
                    """, json.dumps(asdict(result)), result.operation_id)
            except Exception as e:
    ...
    async def list_operations(
        self, 
        status_filter: Optional[BulkOperationStatus] = None,
        limit: int = 50
    ...
    async def cleanup_completed_operations(self, older_than_hours: int = 24):
        ...
        for op_id, progress in self.active_operations.items():
            if (progress.status in [BulkOperationStatus.COMPLETED, BulkOperationStatus.FAILED, 
                                   BulkOperationStatus.CANCELLED] and 
