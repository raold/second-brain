...
class CrossMemoryRelationshipEngine:
    ...
    async def _detect_relationships(self):
        ...
        for i, source_memory in enumerate(memory_list):
            for target_memory in memory_list[i+1:]:
                ...
                relationships = await self._detect_memory_pair_relationships(
    ...
    def _detect_cross_type_relationship(
        self, 
        source: MemoryNode, 
        target: MemoryNode
    ...
    def _calculate_cross_type_strength(
        ...
        total_strength = base_strength + (keyword_bonus * 0.3) + (concept_bonus * 0.2) + (length_factor * 0.1) + type_bonus
        return min(1.0, total_strength)
    
    def _detect_temporal_relationship(
        self, 
        source: MemoryNode, 
        target: MemoryNode
    ) -> Optional[MemoryRelationship]:
        ...
        source_has_temporal = any(re.search(pattern, source.content, re.IGNORECASE) 
                                for pattern in temporal_patterns)
        target_has_temporal = any(re.search(pattern, target.content, re.IGNORECASE) 
                                for pattern in temporal_patterns)
    ...
    def _detect_causal_relationship(
        self, 
        source: MemoryNode, 
        target: MemoryNode
    ) -> Optional[MemoryRelationship]:
        ...
        source_causal = any(re.search(pattern, source.content, re.IGNORECASE) 
                          for pattern in causal_patterns)
        target_causal = any(re.search(pattern, target.content, re.IGNORECASE) 
                          for pattern in causal_patterns)
    ...
    def _extract_keywords(self, content: str) -> Set[str]:
        ...
        stop_words = {
            ...
            'can', 'this', 'that', 'these', 'those', 'from', 'into', 'about'
        }
    ...
    def _extract_concepts(self, content: str, memory_type: str) -> Set[str]:
        ...
        if memory_type == "procedural":
            ...
            patterns = tech_patterns + business_patterns + [
    ...
    def _determine_semantic_relationship_type(
        self, 
        source: MemoryNode, 
        target: MemoryNode
    ) -> RelationshipType:
        ...
        has_hierarchical = any(
            re.search(pattern, source.content + target.content, re.IGNORECASE)
        ...
        causal_patterns = [
            r'\b(cause|effect|result|consequence)\b',
    ...
    def _calculate_cluster_coherence(self, memory_ids: Set[str]) -> float:
        ...
        internal_relationships = [
            ...
            if rel.source_id in memory_ids and rel.target_id in memory_ids
        ]
    ...
    def _generate_relationship_insights(self) -> List[str]:
        ...
        if cross_type_ratio > 0.4:
            insights.append(f"High cross-type connectivity ({cross_type_ratio:.1%}) indicates rich knowledge integration")
        ...
        insights.append(f"Knowledge base is dominated by {dominant_type} memories ({type_counts[dominant_type]} of {len(self.memory_nodes)})")
    ...
    async def get_memory_relationships(self, memory_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "cross_type_count": len([r for r in related_memories 
                                   if r["memory_type"] != memory_node.memory_type])
        }
...
async def demo_cross_memory_relationships():
    ...
    for pattern in analysis.get('cross_type_patterns', []):
        print(f"   {pattern['pattern']}: {pattern['frequency']} relationships (avg strength: {pattern['average_strength']:.3f})")
