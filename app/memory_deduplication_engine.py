...
class MemoryDeduplicationEngine:
    ...
    async def _get_memories_for_deduplication(
        self,
        db,
        memory_filter: Optional[Dict[str, Any]]
    ) -> List[Dict[str, Any]]:
    ...
    async def _find_duplicates_hybrid(
        ...
        for method in [SimilarityMethod.EXACT_MATCH, SimilarityMethod.FUZZY_MATCH, SimilarityMethod.SEMANTIC_SIMILARITY]:
    ...
    def _calculate_performance_metrics(self, result: DeduplicationResult) -> Dict[str, Any]:
        ...
        return {
            ...
            'groups_per_second': result.duplicate_groups_found / result.processing_time if result.processing_time > 0 else 0,
