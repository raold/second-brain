...
class MemoryRelationshipAnalyzer:
    ...
    async def analyze_memory_relationships(
        ...
        async with db.pool.acquire() as conn:
            ...
            candidate_memories = await conn.fetch("""
                ...
                LIMIT $2
            """, memory_id, max_connections * 3)  # Get more candidates for filtering
    ...
    async def _calculate_conceptual_hierarchy(
        ...
        hierarchy_indicators = [
            ('definition', 'example'), ('concept', 'instance'), 
            ('general', 'specific'), ('category', 'item'),
            ('overview', 'detail'), ('summary', 'elaboration')
        ]
    ...
    async def _detect_causal_relationship(
        ...
        causal_words = [
            ...
            'triggers', 'enables', 'prevents', 'influences', 'affects',
    ...
    def _generate_relationship_insights(
        ...
        avg_scores_by_type = {
            rel_type: np.mean(scores) 
            for rel_type, scores in type_scores.items()
        }
        ...
        return {
            'summary': {
                ...
                'strongest_relationship_type': max(avg_scores_by_type.keys(), key=avg_scores_by_type.get) if avg_scores_by_type else None
            },
            'distributions': {
            ...
            'network_characteristics': {
                ...
                'max_secondary_connections': max(
                    [node['relationship_count'] for node in extended_network.values()], 
                    default=0
                )
            }
        }
    ...
    def _extract_themes(self, text: str) -> List[str]:
        ...
        try:
            ...
            # Get top themes
            theme_scores = list(zip(feature_names, tfidf_scores))
