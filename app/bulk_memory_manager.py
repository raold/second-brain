...
class BulkMemoryManager:
    ...
    async def export_memories(
        ...
        try:
            ...
            return ExportResult(
                ...
                processing_time=(datetime.now() - start_time).total_seconds(),
                export_format=format_type.value,
                file_content=f"Export failed: {str(e)}".encode('utf-8')
            )
    ...
    async def _parse_csv(self, data: Union[str, bytes], options: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        reader = csv.DictReader(StringIO(data), delimiter=delimiter) if has_header else csv.reader(StringIO(data), delimiter=delimiter)
            ...
            for memory_data in chunk:
                try:
                    ...
                    memory_id = await db.store_memory(
                        validated_memory['content'],
                        validated_memory['metadata']
                    )
