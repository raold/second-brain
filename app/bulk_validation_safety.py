...
class ContentValidator:
    ...
    async def validate_item(self, item: BulkMemoryItem, level: ValidationLevel) -> ValidationResult:
        ...
        try:
            ...
            return ValidationResult(
                is_valid=len(errors) == 0, errors=errors, warnings=warnings, metadata=metadata, severity=severity
            )

        except Exception as e:
            logger.error(f"Validation failed for item: {e}")
            return ValidationResult(
                is_valid=False, errors=[f"Validation error: {str(e)}"], warnings=[], metadata={}, severity="critical"
            )
    ...
    async def _standard_validation(self, item: BulkMemoryItem) -> ValidationResult:
        ...
        # Metadata validation
        for metadata_field in [item.semantic_metadata, item.episodic_metadata, item.procedural_metadata, item.metadata]:
            if metadata_field:
...
class SafetyEnforcer:
    ...
    async def check_operation_safety(
        ...
        if self.config.enable_rate_limiting and user_id:
            ...
            self.rate_limit_tracker[user_id] = [
                op_time for op_time in self.rate_limit_tracker[user_id] if op_time > hour_ago
            ]
    ...
    async def create_rollback_point(self, operation_id: str, affected_records: list[str]) -> str:
...
class BulkValidationOrchestrator:
    ...
    async def validate_bulk_operation(
        ...
        operation_safety = await self.safety_enforcer.check_operation_safety(operation_type, len(items), user_id)
    ...
    async def _validate_batch(self, batch: list[BulkMemoryItem], level: ValidationLevel) -> list[ValidationResult]:
        """Validate a batch of items concurrently."""
        tasks = [self.content_validator.validate_item(item, level) for item in batch]
    ...
    async def create_operation_checkpoint(self, operation_id: str, affected_records: list[str]) -> str:
