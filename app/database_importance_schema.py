    ...
    try:
        async with pool.acquire() as conn:
            ...
            for i in range(10):
                ...
                """, memory_id, access_types[i % len(access_types)], 
                (i % 5) + 1, str(i % 7))
            ...
            for i, query in enumerate(queries):
                await conn.execute("""
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                """, memory_id, query, i + 1, 0.8 - (i * 0.1), i == 0)
...
async def cleanup_old_access_logs(pool: asyncpg.Pool, days_to_keep: Any) -> Dict[str, Any]:
    ...
    try:
        async with pool.acquire() as conn:
            ...
            logger.info(f"Cleaned up {old_access_count} access logs, {old_search_count} search logs, {old_calc_count} calculation logs")
