...
class ProjectDashboard:
    ...
    def get_recent_completions(self, limit: int = 5) -> List[Dict[str, Any]]:
        ...
        completed_tasks = [
            ...
            if t.status == Status.COMPLETED and t.completed_at
        ]
    ...
    def load_dashboard_data(self):
        ...
        try:
            ...
            if "sprints" in data:
                self.sprints = {
                    k: Sprint(**{**v, "status": Status(v["status"])})
                    for k, v in data["sprints"].items()
                }
            ...
            if "adrs" in data:
                self.adrs = {
                    k: ArchitecturalDecision(**v) for k, v in data["adrs"].items()
                }
