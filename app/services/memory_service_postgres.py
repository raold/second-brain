...
class MemoryServicePostgres:
    ...
    async def search_memories(
        ...
        if search_type in ["vector", "hybrid"] and not self.degradation_manager.is_feature_available("semantic_search"):
            ...
            for memory1_id, memory2_id, similarity in duplicates:
                try:
                    await self.consolidate_memories(
                        source_ids=[memory1_id, memory2_id],
    ...
    async def generate_embeddings_for_all(
        ...
        for memory in memories:
            ...
            if embedding:
                ...
                await self.backend.update_memory(
                    memory["id"],
                    {},
    ...
    async def _check_for_duplicates(
        ...
        if similar:
            logger.warning(f"Found {len(similar)} potential duplicates for memory {memory_id[:8]}...")
    ...
    async def _generate_consolidation_summary(
        self,
