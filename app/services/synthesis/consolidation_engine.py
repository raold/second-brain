...
class MemorySimilarity:
    ...
    def __init__(self, memory1_id: UUID, memory2_id: UUID, similarity_score: float, similarity_type: str):
...
"""
    ...
    async def _fetch_memories_with_embeddings(self, memory_ids: Optional[List[UUID]]) -> List[Dict[str, Any]]:
    ...
    async def _find_similarities(
        ...
        for i in range(len(memory_ids)):
            for j in range(i + 1, len(memory_ids)):
                ...
                similarities.append(MemorySimilarity(
                    memory_ids[i],
                    memory_ids[j],
                    float(sim_score),
                    similarity_type
        ...
        if request.similarity_threshold:
            similarities = [s for s in similarities if s.similarity_score >= request.similarity_threshold]
    ...
    def _create_duplicate_groups(
        self,
        memories: List[Dict[str, Any]],
        similarities: List[MemorySimilarity]
    ) -> List[DuplicateGroup]:
                ...
                if len(current_group) > 1:  # Only groups with 2+ memories
                    ...
                    for i in range(len(group_ids)):
                        for j in range(i + 1, len(group_ids)):
                            for sim in similarities:
                                if (sim.memory1_id == group_ids[i] and sim.memory2_id == group_ids[j]) or \
                                   (sim.memory1_id == group_ids[j] and sim.memory2_id == group_ids[i]):
    ...
    async def _keep_newest_strategy(self, memories: List[Dict[str, Any]]) -> ConsolidationResult:
        ...
        return ConsolidationResult(
            ...
            merge_metadata={
                ...
                'kept_date': kept_memory.get('updated_at', kept_memory.get('created_at')).isoformat(),
    ...
    async def _keep_oldest_strategy(self, memories: List[Dict[str, Any]]) -> ConsolidationResult:
        ...
        sorted_memories = sorted(
            memories,
            key=lambda m: m.get('created_at', datetime.max)
        )
    ...
    async def _merge_content_strategy(self, memories: List[Dict[str, Any]]) -> ConsolidationResult:
        ...
        prompt = self.merge_templates[MergeStrategy.MERGE_CONTENT].format(
            memory1=memory1['content'],
            memory2=memory2['content']
        )
