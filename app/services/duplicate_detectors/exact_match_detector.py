...
class ExactMatchDetector(BaseDuplicateDetector):
    ...
    async def _find_duplicates_impl(
        ...
        for content_hash, memory_group in content_hash_map.items():
            ...
            try:
                group = await self._create_duplicate_group(memory_group, content_hash, config)
                if group:
    ...
    async def _create_duplicate_group(
        self, memories: list[dict[str, Any]], content_hash: str, config: DeduplicationConfig
    ) -> DuplicateGroup:
        """
        ...
        primary_memory_id = self._select_primary_memory(memories, config.merge_strategy.value)
