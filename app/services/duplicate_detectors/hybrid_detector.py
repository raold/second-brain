...
class HybridDetector(BaseDuplicateDetector):
    ...
    def supports_incremental_detection(self) -> bool:
        ...
        return (
            self.exact_detector.supports_incremental_detection()
            and self.semantic_detector.supports_incremental_detection()
        )
    ...
    async def _find_duplicates_impl(
        ...
        combined_groups = await self._combine_detection_results(detection_results, memories, config)
    ...
    def _calculate_combined_score(
        self, detections: list[dict[str, Any]], config: DeduplicationConfig
    ) -> SimilarityScore:
        """
