...
class DeduplicationConfig(BaseModel):
    ...
    similarity_method: SimilarityMethod = Field(
        ...
        description="Method for similarity detection"
    )
    ...
    content_weight: float = Field(
        ...
        ge=0.0, 
        le=1.0, 
        description="Weight for content similarity"
    )
    metadata_weight: float = Field(
        default=0.3, 
        ge=0.0, 
        le=1.0, 
        description="Weight for metadata similarity"
    )
    structural_weight: float = Field(
        default=0.1, 
        ge=0.0, 
        le=1.0, 
        description="Weight for structural similarity"
    )
    
    # Merge and action settings
    merge_strategy: MergeStrategy = Field(
        default=MergeStrategy.SMART_MERGE, 
        description="Strategy for merging duplicates"
    )
    duplicate_action: DuplicateAction = Field(
        default=DuplicateAction.MARK_DUPLICATE, 
    ...
    max_comparisons: Optional[int] = Field(
        default=10000,
        gt=0,
        description="Maximum number of pairwise comparisons"
    )
    ...
    preserve_metadata: bool = Field(
        default=True, 
        description="Preserve metadata from all duplicates"
    )
    create_backup: bool = Field(
        default=True, 
        description="Create backup before deduplication"
    )
    dry_run: bool = Field(
        default=False, 
