...
async def get_memory_relationships(
    ...
    try:
        ...
        return {
            "memory_id": memory_id,
            "target_memory": {
                "content_preview": target_memory['content'][:100] + ("..." if len(target_memory['content']) > 100 else ""),
                "memory_type": target_memory['memory_type'],
...
async def get_memory_clusters(
    ...
    try:
        ...
        filtered_clusters = [
            cluster for cluster in graph_data['clusters'] 
        ...
        for cluster in filtered_clusters:
            ...
            enhanced_cluster = {
                **cluster,
                ...
                ) if member_details else "unknown",
                "avg_importance": np.mean([detail['importance_score'] for detail in member_details]) if member_details else 0
            }
