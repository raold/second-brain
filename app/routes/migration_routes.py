...
from ..migration_framework import MigrationType, MigrationStatus, MigrationConfig
...
async def execute_all_pending(
    ...
    try:
        ...
        # Execute migrations
        results = await engine.execute_pending_migrations(
            migration_config, 
            migration_type
        )
        ...
        for result in results:
            responses.append(MigrationResultResponse(
                migration_id=result.migration_id,
                status=result.status.value,
                start_time=result.start_time,
                end_time=result.end_time,
...
async def get_running_migrations(
    engine: MigrationEngine = Depends(get_migration_engine)
):
