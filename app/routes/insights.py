...
router = APIRouter(
    prefix="/insights", tags=["Insights & Analytics"], dependencies=[Depends(verify_api_key)]
)
...
async def generate_insights(request: InsightRequest, db=Depends(get_db_instance)):
...
async def detect_patterns(request: PatternDetectionRequest, db=Depends(get_db_instance)):
...
async def analyze_clusters(request: ClusteringRequest, db=Depends(get_db_instance)):
...
async def analyze_knowledge_gaps(request: GapAnalysisRequest, db=Depends(get_db_instance)):
...
async def get_learning_progress(
    time_frame: TimeFrame = Query(
        default=TimeFrame.MONTHLY, description="Time frame for progress calculation"
    ),
...
async def get_comprehensive_analytics(
    time_frame: TimeFrame = Query(
        default=TimeFrame.MONTHLY, description="Time frame for analytics"
    ),
    db=Depends(get_db_instance),
):
...
async def get_quick_insights(db=Depends(get_db_instance)):
    """Get quick insights for dashboard display"""
    try:
        engine = AnalyticsEngine(db)
        request = InsightRequest(
            time_frame=TimeFrame.WEEKLY, limit=5, min_confidence=0.7, include_recommendations=True
        )
