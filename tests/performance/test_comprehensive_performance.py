...
class TestV2APIPerformance:
    ...
    async def test_concurrent_api_requests(self, client: AsyncClient, api_key: str):
        ...
        print(
            f"Concurrent requests - Success rate: {success_rate:.2%}, Avg: {avg_response_time:.3f}s, P95: {p95_response_time:.3f}s"
        )
    ...
    async def test_memory_ingestion_performance(self, client: AsyncClient, api_key: str):
        ...
        for size_name, result in results.items():
            print(
                f"  {size_name}: {result['response_time']:.3f}s, {result['content_length']} chars"
            )
...
class TestStructuredDataExtractorPerformance:
    ...
    def test_extraction_performance_by_content_size(self):
        ...
        for mult, result in results.items():
            ...
            print(
                f"  {mult}x ({size_kb:.1f}KB): {result['duration']:.3f}s, {result['extracted_elements']} elements"
            )
    ...
    def test_concurrent_extraction_performance(self):
        ...
        print(
            f"Concurrent extraction - Success: {success_rate:.2%}, Avg: {avg_duration:.3f}s, Max: {max_duration:.3f}s"
        )
...
class TestWebSocketPerformance:
    ...
    async def test_connection_establishment_performance(self):
        ...
        print(
            f"Connection establishment - {len(connections)} connections in {metrics['duration_seconds']:.3f}s"
        )
    ...
    async def test_broadcast_performance(self):
        ...
        print(
            f"Broadcast performance - 10 broadcasts to 50 connections in {metrics['duration_seconds']:.3f}s"
        )
    ...
    async def test_connection_cleanup_performance(self):
        ...
        for i in range(100):
            ...
            await self.websocket_service.connection_manager.connect(mock_ws, conn_id, f"user-{i}")
    ...
    async def test_concurrent_operations_performance(self):
        ...
        async def establish_and_broadcast():
            ...
            await self.websocket_service.send_notification(
                f"user-{conn_id}", {"type": "test", "message": "concurrent test"}
            )
    ...
    async def test_websocket_memory_efficiency(self):
        ...
        for cycle in range(10):
            ...
            await self.websocket_service.event_broadcaster.broadcast_event(
                "memory.test", {"cycle": cycle, "connections": len(connections)}
            )
        ...
        print(
            f"WebSocket memory efficiency - {memory_increase:.1f}MB increase over 200 connection cycles"
        )
...
class TestCrossComponentPerformance:
    ...
    async def test_end_to_end_performance(self, client: AsyncClient, api_key: str):
        ...
        print(
            f"End-to-end performance - {len(results)} operations in {metrics['duration_seconds']:.3f}s, {success_rate:.2%} success"
        )
    ...
    def test_system_resource_utilization(self):
        ...
        for i in range(50):
            ...
            if i % 10 == 0:
                asyncio.run(
                    websocket_service.event_broadcaster.broadcast_event(
                        "system.test", {"iteration": i, "extracted": len(result.key_value_pairs)}
                    )
                )
        ...
        print(
            f"Resource utilization - CPU: +{cpu_increase:.1f}%, Memory: +{memory_increase:.1f}%, Duration: {metrics['duration_seconds']:.3f}s"
        )
