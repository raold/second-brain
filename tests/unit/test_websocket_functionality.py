...
class TestWebSocketModels:
    ...
    def test_broadcast_message_model(self):
        ...
        message = BroadcastMessage(
            event_type="test.event", payload={"key": "value"}, priority=EventPriority.HIGH
        )
    ...
    def test_broadcast_message_with_data_field(self):
        ...
        message = BroadcastMessage(event_type="test.event", data={"key": "value"})
    ...
    def test_websocket_message_model(self):
        ...
        message = WebSocketMessage(type="event", event=event, success=True)
    ...
    def test_subscription_request_validation(self):
        ...
        with pytest.raises(ValueError):
            SubscriptionRequest(
                event_types=["test.event"], action="invalid_action"  # Should fail validation
            )
...
class TestConnectionManager:
    ...
    async def test_broadcast_message(self):
        ...
        mock_ws1 = AsyncMock()
        mock_ws2 = AsyncMock()
        self.manager.active_connections = {"conn-1": mock_ws1, "conn-2": mock_ws2}
    ...
    async def test_broadcast_with_failed_connection(self):
        ...
        self.manager.active_connections = {"conn-good": mock_ws_good, "conn-bad": mock_ws_bad}
...
class TestWebSocketIntegration:
    ...
    async def test_broadcast_to_multiple_connections(self):
        ...
        # Broadcast an event
        await service.event_broadcaster.broadcast_event("memory.created", {"memory_id": "mem-123"})
    ...
    async def test_user_specific_notifications(self):
        ...
        service.connection_manager.active_connections = {"conn-1": user1_conn, "conn-2": user2_conn}
        service.connection_manager.user_connections = {"user-1": {"conn-1"}, "user-2": {"conn-2"}}
...
class TestWebSocketSecurity:
    ...
    async def test_input_sanitization(self):
        ...
        mock_websocket.receive_text.side_effect = [malicious_message, WebSocketDisconnect()]
    ...
    def test_subscription_validation(self):
        ...
        # Valid subscription
        valid_sub = SubscriptionRequest(event_types=["memory.created"], action="subscribe")
        assert valid_sub.action == "subscribe"

        # Invalid action should raise validation error
        with pytest.raises(ValueError):
            SubscriptionRequest(event_types=["memory.created"], action="invalid")
