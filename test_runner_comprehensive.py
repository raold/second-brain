...
class ComprehensiveTestRunner:
    ...
    async def test_basic_imports(self):
        ...
        try:
            ...
            # Test docs imports
            from app.docs import Priority, MemoryType
    ...
    async def test_mock_database(self):
        ...
        try:
            ...
            self.results.append(TestResult("Mock Database", False, f"Database test failed: {e}", traceback.format_exc()))
    ...
    async def test_app_configuration(self):
        ...
        try:
            ...
            self.results.append(TestResult("Middleware Setup", True, f"{middleware_count} middleware components loaded"))
            ...
            self.results.append(TestResult("App Configuration", False, f"Configuration test failed: {e}", traceback.format_exc()))
        ...
        try:
            ...
            async with AsyncClient(app=app, base_url="http://test") as client:
                ...
                if response.status_code == 200:
                    data = response.json()
                    self.results.append(TestResult("Health Endpoint", True, f"Status: {data['status']}, Version: {data['version']}"))
                else:
    ...
    async def test_dashboard_system(self):
        ...
        try:
            ...
            self.results.append(TestResult("Dashboard System", False, f"Dashboard test failed: {e}", traceback.format_exc()))
    
    async def test_memory_visualization(self):
        """Test memory visualization features"""
        try:
            # Test visualization routes import
            from app.routes.visualization_routes import visualization_router
            ...
            self.results.append(TestResult("Graph Generation", True, f"Generated graph with {len(graph_data.get('nodes', []))} nodes"))
            
        except Exception as e:
            self.results.append(TestResult("Memory Visualization", False, f"Visualization test failed: {e}", traceback.format_exc()))
    
    async def test_performance_benchmarks(self):
        """Test performance benchmark functionality"""
        try:
            from tests.performance.test_performance_benchmark import PerformanceMetrics, BenchmarkResult
            
            # Test performance classes
            metrics = PerformanceMetrics(
                response_time=0.1, memory_usage=50.0, cpu_usage=25.0,
                database_connections=1, error_rate=0.0, throughput=100.0
            )
            self.results.append(TestResult("Performance Metrics", True, "Performance metrics class working"))
            
            result = BenchmarkResult(
                test_name="test", endpoint="/test", method="GET",
                avg_response_time=0.1, min_response_time=0.05, max_response_time=0.2,
                p95_response_time=0.15, p99_response_time=0.18, throughput=100.0,
                error_rate=0.0, success_rate=100.0, memory_usage=50.0,
                cpu_usage=25.0, total_requests=10, passed=True
            )
            self.results.append(TestResult("Benchmark Result", True, "Benchmark result class working"))
            
        except Exception as e:
            self.results.append(TestResult("Performance Benchmarks", False, f"Performance test failed: {e}", traceback.format_exc()))
    
    async def test_security_features(self):
        """Test security features"""
        try:
            from app.security import SecurityMiddleware, verify_api_key
            ...
            self.results.append(TestResult("Security Features", False, f"Security test failed: {e}", traceback.format_exc()))
    
    async def test_version_management(self):
        """Test version management"""
        try:
            from app.version import get_version_info, VERSION, BUILD_TYPE
            ...
            self.results.append(TestResult("Version Management", False, f"Version test failed: {e}", traceback.format_exc()))
